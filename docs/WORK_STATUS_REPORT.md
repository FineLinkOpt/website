...
## 2026-07-01 Hero 色罩改回純色平塗,修正誤用暗模式色階

- 業主詢問 hero 綠色底是否符合品牌規範,稽核 `assets/files/FineLink-Design-Language-v1.md` 後發現本次編輯新增了兩個未揭露的偏差(先前業主已認可的只有「飽和色大面積作背景」這項):
  1. 色罩改用了 `linear-gradient(180deg, ...)` 上下漸層,違反規範第5節硬性規則「無漸層。背景、文字、按鈕一律純色平塗」。
  2. 漸層兩端分別用了 `--fl-green-400`(#33B798)與 `--fl-green-500`(#1D8A64),但規範色彩表明訂 400 為「暗模式主色」、500 為「亮模式主色」,本頁是亮色頁面卻疊加了暗模式專用色階。
- 業主確認改回純色,並詢問色階是否合規。已改為單一色階 `--fl-green-500`(#1D8A64,規範表定亮模式主色,也是先前那次已核准之純色平塗所用的同一色階),透明度 0.82 讓照片透出。
- CSS 實作備註:純色值無法直接疊在背景圖片之上(`background` 簡寫的非最終圖層必須是合法 `<image>`),故技術上仍以 `linear-gradient(rgba(...), rgba(...))` 語法承載,但頭尾為同一色值,視覺上零色階過渡,等同純色平塗,已於 `index.html`、`3d-profiler.html` 加註解說明此為語法限制而非設計漸層。

## 2026-07-02 CLAUDE.md 設計 Token 段落更新至翡翠綠規範

- 發現 `CLAUDE.md` 的「設計 Token 參考」段落仍描述已淘汰的舊設計(深海軍藍底 `#050b14`、螢光青主色 `#00e5ff`、玻璃擬態、漸層字),與 2026-07-01 完成的翡翠綠 rebrand 矛盾;若後續依該段文件作業會被帶回舊風格。
- 已改寫該段落為現況:token 唯一來源 `assets/finelink-tokens.css`(經 `assets/styles.css` @import 載入,與品牌資料夾 `公司品牌/04_UI/finelink-tokens.css` 為同一份,需雙邊同步)、主色翡翠綠 `--fl-primary`(亮 #1D8A64 / 暗 #33B798)、字體 IBM Plex Sans + Noto Sans TC + IBM Plex Mono、硬規則(無漸層/毛玻璃/發光/陰影)。
- 同時把兩項「已知刻意偏差」寫進 CLAUDE.md,避免被誤修正:卡片邊框 1px(非規範 0.5px,業主已確認)、hero 色罩之同色 linear-gradient 語法(純色平塗的技術性寫法)。
- 本次僅動 `CLAUDE.md` 與本紀錄,未動任何頁面或資產。

## 2026-07-03 「關於佳聯」內文微調

- 業主要求調整首頁「關於佳聯」段落開頭文案。將「佳聯光學科技的股東與工程團隊，出身光學材料大廠的光學技術部門，累計四十年以上的光學與設備開發經驗。」改為「佳聯光學科技的工程團隊，出身於光學材料大廠的技術部門，累計超過四十年的光學與設備開發經驗。」（移除「股東與」、「光學技術部門」改「技術部門」、「四十年以上」改「超過四十年」）。
- 僅動 `index.html` 一處內文（約第 267 行），其餘不變。

## 2026-07-07 Hero 等高線改用壽山品牌署名母題

- 品牌已於 `公司品牌/04_UI/FineLink-HMI-Guidelines-v1.md`（第 4.品牌識別錨點「畫布等高線署名」）把等高線母題規範為**高雄壽山（Shoushan）真實地形等高線**（佳聯位於高雄,以在地地形署名）。業主要求把首頁 hero 的裝飾等高線也換成同款。
- 資料來源:壽山 SRTM1（30m,NASA/USGS public domain）自算等值線,已烤成 `3DVision/resources/brand/shoushan_contours.json`（正規化座標,minor 15m / major 60m 分主次線,含最高峰座標）。此為 HMI 端資產,非本 repo,僅取其幾何。
- 實作:以離線腳本讀該 JSON、把等高線**旋轉 90°**（原始為縱長山形,旋轉後貼合寬幅 hero）,採「峰置中、填滿寬度」構圖（peak 對齊畫布中心 600,300,大等高環上緣出血),映射進既有 `viewBox="0 0 1200 600"` 的 `.hero-topo` SVG。**沿用原本的視覺樣式不變**:白線、`stroke-opacity` 次線 0.32 / 主線 0.62、`stroke-width` 0.7 / 1.4——僅替換幾何,不改 hero 綠底色罩與線條配色。
- 業主於三種構圖(A 全山置中 / B 高聳出血 / C 峰置中大環)中選 C,並同意旋轉 90°。
- 動到:`index.html` 的 hero-topo `<svg>` 內容(原手繪不規則波浪線 → 壽山等高線,19 條次線 + 7 條主線,含來源註解),以及第 72 行 hero CSS 註解同步改為「壽山署名等高線」。其餘頁面與資產不變。
- 署名合規(見 HMI 規範第 4 節存證):SRTM1 屬公有領域、等高線為自算非描摹地圖、未使用「壽山」字樣或官方標誌,僅作抽象裝飾母題。

## 2026-07-07 3D 產品頁 hero 等高線同步為壽山母題

- 承上（首頁 hero 已改用高雄壽山品牌署名等高線），業主要求 `3d-profiler.html`（Topo-Engine 產品頁）的 hero 也改成同款壽山等高線,與首頁一致。
- 實作:直接沿用首頁 `index.html` 內已烤好的壽山 `.hero-topo` `<svg>` 幾何（19 條次線 + 7 條主線,旋轉 90°、峰置中構圖,含來源註解）,整段替換 `3d-profiler.html` 原本手繪的「白色不規則等高線」SVG。白線配色與 `stroke-opacity`（次線 0.32 / 主線 0.62）、`stroke-width`（0.7 / 1.4）樣式與首頁完全一致。
- 動到:`3d-profiler.html` 的 hero-topo `<svg>` 內容,以及第 70 行 hero CSS 註解同步改為「壽山署名等高線」。hero 綠底色罩、標題文案、其餘頁面與資產不變。

## 2026-07-07 hero 等高線主線調淡至 0.40（兩頁）

- 業主檢視後要求把壽山等高線的**主線再淡一點**。曾試做「平滑化（Catmull-Rom 曲線）＋點位簡化（Douglas–Peucker）」讓線條更圓潤,但業主決定不採用,**回復到最初的壽山直線等高線幾何**（19 條次線 + 7 條主線,原始 DEM 資料點,無曲線平滑）。
- 唯一保留的調整:主線 `stroke-opacity` 由 0.62 → **0.40**（次線維持 0.32、白線配色與 stroke-width 0.7/1.4 不變）。
- `index.html` 與 `3d-profiler.html` 兩頁 hero-topo `<svg>` 同步為相同幾何與配色。其餘頁面、資產、hero 綠底色罩不變。
