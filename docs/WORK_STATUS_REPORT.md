...
- 修正 `3d-profiler.html` 中「直覺的 3D 視覺化分析」區塊第一張圖 (3D拼接圖) 的文案，將原先不符的「元件缺陷與高度落差分析」更新為「大視野 3D 拼接分析」，並改寫說明以強調突破視野限制與呈現大尺寸全貌的優勢。
...
- 再次優化 Hero 設備介紹區塊的文案，將「專為極端應用打造」改為「專為嚴苛檢測需求打造」，並將說明改寫為「完美應對偏光板的翹曲變形與記憶體模組的複雜結構，實現高穩定度的精密量測」，提升產品的工業級信任感。
