...
- 移除「工業級精度規格」中的「局部雜訊穩定度」項目，使焦點更集中於 Cg 能力指數與靜態重複精度。
- 依據使用者回饋，還原 3d-profiler.html 的文字排版，將「軟體核心功能與演算法」與「工業級精度規格」由條列式恢復為卡片網格形式，並補回「局部雜訊穩定度」規格。
- 移除 index.html 中 3D 輪廓量測區塊的「了解更多」引導文字。
